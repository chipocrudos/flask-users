...
def login_view():
    ...
    login_user = UserModel.query.filter_by(email=user["email"], is_active=True).first()

    if not login_user:
        return make_response(
            jsonify("User or password incorrect"), HTTPStatus.BAD_REQUEST
        )
...
def registration_user_view():
    ...
    token = urlsafe.dumps(
        email_schema.dump(db_user), salt=Configuration.USERS_ACTIVATE_SALT
    )
    send_email(
        "registration_user",
...
@validate_url_token(
    Configuration.USERS_ACTIVATE_SALT, Configuration.USERS_ACTIVATE_TOKEN_AGE_IN_SECONDS
)
...
def active_user_view(payload):
    ...
    return make_response(jsonify("User active successfully"), HTTPStatus.OK)


@validate_url_token(
    Configuration.USERS_RESET_PASSWORD_SALT,
...
def change_password_view(payload):
    ...
    return make_response(jsonify("Your password has by updated"), HTTPStatus.OK)
...
def password_recovery_view():
    ...
    if exist_user and exist_user.is_active:
        token = urlsafe.dumps(email, salt=Configuration.USERS_RESET_PASSWORD_SALT)
        send_email(
            "recovery_password",
            "Password recovery",
            exist_user,
    ...
    return make_response(jsonify("Email has by sended with instruction"), HTTPStatus.OK)
